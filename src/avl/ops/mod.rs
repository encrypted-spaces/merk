    ...
    S: Fetch + Sized + Send + Clone,
{
    ...
    ///
    /// Keys in batch must be sorted and unique. `DeleteRange` is handled by
    /// `InMemoryMerk`, which splits mixed batches into range deletes and
    /// point-op segments before calling this.
    #[cfg(not(use_box))]
    pub fn apply_cow(
    ...
    fn build(batch: &mut Batch, source: S) -> Result<Option<Node>> {
        ...
        let mid_tree = match mid_op {
            ...
            DeleteRange(_) => {
                // Mixed batches are split by InMemoryMerk before this point-op
                // recursion sees them.
                unreachable!("DeleteRange should not appear in point-op segments")
            }
        };
    ...
    fn apply_mut(self, batch: &mut Batch) -> Result<(Option<Self>, LinkedList<Vec<u8>>)> {
        self.record_visit_node(self.tree());
    ...
    /// Returns an opened child's balance factor when a rotation decision needs
    /// it. A pruned child has only metadata; using that metadata as an internal
    /// rotation oracle would make sparse write replay trust unopened proof
    /// data.
    #[inline]
    fn child_balance_factor_for_rotation(&self, left: bool) -> Result<i8> {
        match self.tree().child_ref(left) {
            Some(Child::Resident(child)) => {
                self.clone_source().record_visit(child);
                Ok(child.balance_factor())
            }
            Some(child @ Child::Pruned(_)) => {
                let source = self.clone_source();
                let child = source.fetch(child)?;
                source.record_visit(&child);
                Ok(child.balance_factor())
            }
            None => Err(Error::Tree(format!(
                "AVL rotation decision missing {} child",
    ...
    fn maybe_balance(self) -> Result<Self> {
        ...
        let heavy_child_bf = self.child_balance_factor_for_rotation(left)?;
        let tree = if left == (heavy_child_bf > 0) {
    ...
    match op {
        ...
        DeleteRange(_) => {
            return Err(Error::Key(
                "apply_in_place does not support DeleteRange".into(),
            ));
        }
    }
